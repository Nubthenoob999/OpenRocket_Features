...
public final class HeadlessOrkSimulationRunner {
	...
	public static OrkSimulationResult runSimulationDetailed(Simulation simulation)
		...
		return new OrkSimulationResult(
				...
				describePhysicsAeroRuntime(options),
				describeTableSource(options),
				maxFinite(mach));
	}
	...
	private static String describePhysicsAeroRuntime(SimulationOptions options) {
		if (options == null) {
			return "";
		}
		...
		return "PHYSICS_AERO_" + options.getPhysicsAeroMode().name();
	}

	private static String describeTableSource(SimulationOptions options) {
		if (options == null) {
			return "";
		}
		if (!options.getPhysicsAeroSettings().isEnabled()) {
			return "TABLE_DISABLED";
		}
		if (options.getPhysicsAeroSettings().getTableContentHash().isBlank()) {
			return "TABLE_IDENTITY_MISSING";
		}
		return "TABLE_CACHE_IDENTITY";
	}
	...
	public static final class OrkSimulationResult {
		...
		private OrkSimulationResult(TelemetrySeries series,
			...
			this.physicsAeroMode = physicsAeroMode;
		...
		/** Compatibility accessor for older report consumers. */
		@Deprecated
		public String getRomMode() {
			return getPhysicsAeroMode();
		}
