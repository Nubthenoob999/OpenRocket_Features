...
public final class PhaseThreeSingleSimulationRunner {
	...
	public static PhaseTwoDatasetResult compareCurrentSimulation(OpenRocketDocument document,
		...
		PhaseThreeNativeAirbrakesConfigurer.forceTableRuntime(candidateSimulation.getOptions(), null);
		...
		return new PhaseTwoDatasetResult(
				...
				"CURRENT_SIMULATION",
				provenance(document),
				candidateTelemetry.getPhysicsAeroMode(),
				candidateTelemetry.getTableSource(),
	...
	private static String classifyDataset(TelemetrySeries reference) {
		return reference.getSchema() == TelemetrySchema.AB_IMU_INTERLEAVED ? "BROKEN" : "TABLE_USABLE";
	}
}
