...
public record BoundaryLayerConfiguration(TransitionMode transitionMode, double turbulencePercent,
		...
		SkinFrictionCompressibilityMode skinFrictionCompressibilityMode) {
	public static BoundaryLayerConfiguration defaults() {
		return new BoundaryLayerConfiguration(TransitionMode.NATURAL, 0.2, 0.02, 2.0,
				WallThermalBoundary.ADIABATIC, 1.4, 0.72, 1e-3, 2e-3,
				SkinFrictionCompressibilityMode.VAN_DRIEST_II_TRANSITION);
	}
	public BoundaryLayerConfiguration {
		if (transitionMode == null || wallMode == null || skinFrictionCompressibilityMode == null
				|| turbulencePercent < 0 || transitionBlendLengthM <= 0
