package info.openrocket.core.simulation.montecarlo;

import java.util.ArrayList;
import java.util.Comparator;
import java.util.List;

/** Descriptive statistics for one scalar Monte Carlo output. */
public final class MetricStatistics {
	private final List<Double> sortedValues;
	private final double mean;
	private final double standardDeviation;
	private static final double NORMAL_95_PERCENT_CRITICAL_VALUE = 1.959963984540054;

	private MetricStatistics(List<Double> values) {
		if (values.isEmpty()) {
			throw new IllegalArgumentException("At least one finite value is required");
		}
		List<Double> finiteValues = new ArrayList<>(values.size());
		for (double value : values) {
			if (Double.isFinite(value)) {
				finiteValues.add(value);
			}
		}
		if (finiteValues.isEmpty()) {
			throw new IllegalArgumentException("At least one finite value is required");
		}
		finiteValues.sort(Comparator.naturalOrder());
		this.sortedValues = List.copyOf(finiteValues);

		// Welford's online algorithm avoids the cancellation inherent in E[x^2] - E[x]^2.
		double runningMean = 0;
		double sumSquaredDeviations = 0;
		int count = 0;
		for (double value : sortedValues) {
			count++;
			double delta = value - runningMean;
			runningMean += delta / count;
			double updatedDelta = value - runningMean;
			sumSquaredDeviations += delta * updatedDelta;
		}
		this.mean = runningMean;
		this.standardDeviation = count > 1
				? Math.sqrt(Math.max(0, sumSquaredDeviations) / (count - 1.0)) : 0;
	}

	public static MetricStatistics from(List<Double> values) {
		return new MetricStatistics(values);
	}

	public int getSampleCount() {
		return sortedValues.size();
	}

	public double getMinimum() {
		return sortedValues.get(0);
	}

	public double getMaximum() {
		return sortedValues.get(sortedValues.size() - 1);
	}

	public double getMean() {
		return mean;
	}

	public double getMedian() {
		int upperMiddle = sortedValues.size() / 2;
		if (sortedValues.size() % 2 != 0) {
			return sortedValues.get(upperMiddle);
		}
		double lowerValue = sortedValues.get(upperMiddle - 1);
		double upperValue = sortedValues.get(upperMiddle);
		return lowerValue + (upperValue - lowerValue) / 2.0;
	}

	public double getStandardDeviation() {
		return standardDeviation;
	}

	/**
	 * Estimated Monte Carlo standard error of the sample mean.
	 */
	public double getStandardError() {
		return standardDeviation / Math.sqrt(sortedValues.size());
	}

	/**
	 * Normal-approximation 95% confidence interval for the population mean.
	 * This interval describes uncertainty in the mean, not the spread of individual runs.
	 */
	public ConfidenceInterval getMeanConfidenceInterval95() {
		double margin = NORMAL_95_PERCENT_CRITICAL_VALUE * getStandardError();
		return new ConfidenceInterval(mean - margin, mean + margin);
	}

	/** Return an empirical nearest-rank quantile. */
	public double getQuantile(double probability) {
		if (!(probability > 0) || probability > 1) {
			throw new IllegalArgumentException("Probability must be in the range (0, 1]");
		}
		int index = Math.max(0, (int) Math.ceil(probability * sortedValues.size()) - 1);
		return sortedValues.get(index);
	}

	public record ConfidenceInterval(double lowerBound, double upperBound) {
	}
}
