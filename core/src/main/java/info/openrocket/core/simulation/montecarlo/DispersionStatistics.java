...
public final class DispersionStatistics {
	...
	private DispersionStatistics(List<LandingPoint> points) {
		List<LandingPoint> finitePoints = new ArrayList<>(points.size());
		for (LandingPoint point : points) {
			if (Double.isFinite(point.east()) && Double.isFinite(point.north())) {
				finitePoints.add(point);
			}
		}
		if (finitePoints.isEmpty()) {
			throw new IllegalArgumentException("At least one finite landing point is required");
		}

		this.sampleCount = finitePoints.size();
		double eastTotal = 0;
		double northTotal = 0;
		for (LandingPoint point : finitePoints) {
