...
public class HailMaryLaunchRegressionTest {
	...
	@Test
	public void hailMaryRomRunStaysNearTelemetryAndSavedBaseline() throws Exception {
		...
		assertTrue(Math.abs(rerunFeet - actualFeet) < 120.0,
				"Expected rerun to stay close to telemetry. actual=" + actualFeet + " rerun=" + rerunFeet);
		assertTrue(Math.abs(rerunFeet - savedFeet) < 120.0,
	...
	private static Simulation loadSource() throws Exception {
		...
		for (Simulation simulation : document.getSimulations()) {
			...
			if (simulation.getOptions().getRomSurfaceMode() == null
					|| !simulation.getOptions().getRomSurfaceMode().name().equals("FOUR_D")) {
				continue;
			}
